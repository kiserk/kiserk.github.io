...
### `#intro` panel pass (2026-08-03)
- **Problem:** `#intro` is the only bare text-on-water section (everything else gets a `.surface` card). `.water-scrim` is `position: fixed`, so its dark `0.55` ends only cover the top/bottom of the *viewport* — a `min-h-screen` section's centred copy lands in the flat `0.24` middle band, over the brightest foam. Measured worst-pixel contrast across the 20 s loop: proof points **3.4**, "Open to…" **3.2** (both fail AA); descriptor 5.4.
- **Second cause:** the muted tiers used Tailwind element `opacity-70`/`opacity-60`, which fades the protective `text-shadow` along with the glyphs — the faintest copy got the weakest shadow, and `#intro` ignored the `--ink-*` tokens the rest of the site uses.
- **Fix:** new `.scrim-focus` component class in `global.css` — a `::before` radial vignette (`120% 90% at 6% 50%`, `0.58 → 0.40 @52% → 0 @84%`, `rgba(4,8,14,…)`), plus a re-centred variant under 768px. Applied to `#intro` only. Element opacities replaced with `text-[color:var(--ink-muted)]` / `--ink-faint`; CTA pills use `transition-colors` + `hover:text-[color:var(--ink)]` (had to drop the inline `color: inherit`, which was beating the class). `--text-shadow` gained a tight `0 1px 2px` layer — a 14 px blur alone can't define an 11 px glyph.
- **Result** (empirical: copy hidden, real composited background screenshotted, contrast computed per pixel across 11 frames × 4 configs): every tier ≥ **4.7:1** at the worst pixel; typical 6–14:1. Holds at 1280/1440/390 wide and on the reduced-motion poster still.
- **Tuning note:** the peak alpha is the knob. `0.55/0.32@45/0@78` measured 4.50 — passing but zero margin; the shipped `0.58/0.40@52/0@84` buys the headroom mostly by *holding the mid-tone across the text column*, not by darkening. Don't drop below ~0.5 without re-measuring.
- **Follow-up — lighter ink (site-wide):** `--ink-muted` 0.82 → **0.90**, `--ink-faint` 0.66 → **0.78**, keeping three distinct steps against `--ink` 0.96. Deliberately a token change, not an `#intro` override, so the intro stays consistent with the cards (~100 usages across every section plus contact/privacy/swell). Safe by construction: the cards are dark glass, so raising text alpha only ever increases their contrast. `#intro` re-measured after: proof points 5.19 → **6.54**, "Open to…" 5.07 → **6.37** at the worst pixel.
- **Follow-up — seam fix:** the ellipse is `90%` of the section height, so it was still ~**0.37 alpha where it hit the section's top/bottom edges** and got cut off dead — a visible horizontal line against `#top` and `#projects` (measured: alpha stepped 0.000 → 0.364 in a single pixel row). Fixed with `mask-image: var(--scrim-focus-fade)`, a vertical `linear-gradient` feathering over **8rem** at each end. 8rem = `py-32`, which is the smallest clear gap above/below the copy at any breakpoint (measured 128px at 360×640; 147–286px everywhere else) — so the shade is at full strength wherever text actually sits and the contrast numbers are unchanged. Prefixed `-webkit-mask-image` for Safari < 15.4.
...
### Whiter ink + feathered panel edges (2026-08-03)
- **Ink is now one hue at three alphas.** `--ink-muted` and `--ink-faint` used to be blue-tinted (236,240,246 / 228,236,245) against a warm `--ink` (246,244,240) — the hue mismatch, not just the level, is what made them read as cool grey. All three now share the warm near-white; hierarchy comes from alpha alone (0.96 / 0.92 / 0.82). `#intro` worst-pixel contrast after: descriptor 10.8, proof points 7.8, "Open to…" 7.6.
- **`.text-wash` replaced the per-panel glass cards.** Two failed attempts first, both for the same reason: *any* per-panel background gives the eye a box to find, and a box with a soft edge just reads as a blurred rectangle no matter how long the fade (tried 0.72 fill/1.25rem, then 0.46/5.5rem — both looked wrong).
- **What works:** one continuous pool of shade on `<main>`, centred on the text column — the same language as the hero's `.scrim-focus`, with no shapes anywhere. `linear-gradient(to right, transparent, rgba(4,8,14,0.5) calc(50% - 24rem), … )`: the horizontal profile is constant down the page, so a single element covers content of any height and there are no seams between sections. 24rem either side of centre holds full strength across the `max-w-[720px]` column; below ~768px the `calc()` stops collapse and the wash goes flat, which is what full-width mobile copy wants. Vertical `--text-wash-fade` (5rem) meets the hero above and the footer below at zero.
- **Still `.surface`:** the `/swell` `<select>`, the Leaflet map container, and the `_golden-hour` pill — real UI containers that need a defined edge.
- **Verified:** 228 text elements inside `<main>` across home/contact/privacy/swell, over multiple clip frames — all pass, lowest 6.29. Harness note: hide the fixed `header`/`#brand-home` when screenshotting backgrounds, and re-read rects at each scroll position (lazy thumbnails resize panels) — both caused phantom ~1.1:1 failures.

### Sticky headings + abstracts disclosure (2026-08-03)
- **`.sticky-title` / `.sticky-subtitle`** pin the section title and the current subsection label. Three things this needs, none obvious:
  1. **`body { overflow-x: hidden }` silently breaks `position: sticky`** — `hidden` makes body a scroll container. Changed to `overflow-x: clip` (with an `@supports` fallback), which suppresses the overflow without creating one.
  2. **A sticky element only travels inside its containing block.** Projects/CV had their `<h1>` inside a `<header>` sized to its own contents, so it had nowhere to go. Those wrappers were removed and the headings are now direct children of the tall section body. Don't reintroduce a wrapper.
  3. **`--sticky-top: 5.5rem`** clears the fixed brand (runs to y=75) and header (y=60). Horizontal clearance can't be used: at ≤1024px the content column slides under the brand.
- **Backdrops:** the label's z-index must be *above* the title's (21 vs 20) — the title's backdrop overshoots past the point where the label pins, so a lower label gets dimmed by it. Both bands overshoot their boxes by 1.5rem so they overlap instead of leaving a gap for copy to show through; the flat portion is fully opaque because even 2% leaks readable ghost text. Masked horizontally on the same column as `.text-wash` so they read as a wash, not a bar.
- **Conference abstracts:** only the 2 first-author ones show; the 6 co-authored sit behind a `<details>` disclosure (works with no JS, keyboard/SR-native). Card markup extracted to `src/components/AbstractCard.astro` so the two lists can't drift.
