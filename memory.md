...
- **`.surface-soft`** — the card panels' edges now dissolve instead of ending in a crisp 1px border. Background + blur live on a `::before` because **a mask applies to an element's children too**, so masking the panel directly would fade the copy with it. No border on the feathered variant: the mask is zero along the perimeter, exactly where a border sits, so it would be invisible anyway. Feather is `--surface-feather: 1.25rem`, chosen to sit inside the smallest panel padding in use (`p-6` on the publication cards).
